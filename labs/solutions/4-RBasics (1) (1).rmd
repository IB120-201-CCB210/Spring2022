...
plot(mtcars$hp, mtcars$mpg) # defaults to scatter plot '$' is used for extract 
...
b <- rnorm(100, mean = 50, sd = 100)
hist(a, freq = FALSE, ylim = c(0,0.05)) 
hist(b, freq = FALSE, ylim = c(0,0.01)) 
#As sds get bigger, the number generated would be more scattered and widely spread (larger range)
```
...
```{r q6}
z = pnorm(x, mean = 50, sd = 5)
plot(x, z)
abline(h=0.5, v=50) 
#far-most right point means at this point the cumulative densities have reached 1, and this corresponds to the largest number we assigned.
```
...
```{r q7}
#When we reach the mean value, the PDF reaches to its largest value, while CDF should reach 0.5 since half of the probability is now added up. No matter the value of sd, the principle mentioned above won't change. If the sd gets bigger, the CDF should look more plain, which corresponds to the PDF (since in PDF, points are scattered around, it would me much more difficult for CDF to grow/cumulate than those with smaller sd).
```
