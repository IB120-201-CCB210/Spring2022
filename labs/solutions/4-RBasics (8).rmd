...
#pnorm(mean(a), mean = mean(a), sd = sd(a))
...
c <- rnorm(100, mean = 50, sd = 100)
hist(c, freq = FALSE)

# spread in x axis: b has a narrower spread (domain) in x axis than c
# height in y axis: b has higher frequency (or density? not sure) around the mean
# so larger sd causes the data to be more wide-spread
...
```{r q4}
d <- rnorm(100, mean = 50, sd = 5)
dnorm(d, mean = 50, sd = 5)
...
```{r q6}
e <- pnorm(d, mean = 50, sd = 5)
plot(d, e)
abline(h=0.5, v=50)

# the far-most right point: its x value is the largest sample value we got here and its y value is 1, which means the cumulative density reaches 1 (100%)

```

7. Under your plot function from part 6, type in the following code: \begin{equation*} abline(h=0.5, v=50)  \end{equation*} 
This should have created grid lines on your plot. In trying to understand the relationship between PDFs and CDFs, how does the point in the graph outlined by the gridlines correspond to your answer in part 2. Answer in comments.

```{r q7}

#the intersect of two lines are the mean of this ramdom sample from normal distribution
#the cumulative density here is 0.5, which means half of the points are smaller and the other half are larger than this point in their x-value
#also by eyeballing the plot we can see 50% of dots are in the bottomleft area while the other 50% are in the topright, which is consistent with the cumulative density here

#to talk more about the relationship between pdf and cdf, we can see that the slope at the intersect (the mean) in cdf is the steepest, and the y-value (density) of the mean in pdf (shown in Q5) is also the larget; this really corresponds to the fact that pdf is the derivative of cdf
