...
length(which(samples<=mean(samples))) / length(samples) # this is the proportion of samples to the left of the mean of samples
...
```{r q3}
samples2 <- rnorm(100, mean = 50, sd = 10)
samples3 <- rnorm(100, mean = 50, sd = 100)

hist(samples2)
hist(samples3)

# as SD increases, the data are drawn from a larger range, but the distribution maintains a bell-like curve
```
...
densities <- dnorm(samples, mean = 50, sd = 5)
...
plot(sort(samples), (1:length(samples))/length(samples), type="l", ylab = "cumulative density", xlab = "value")
points(samples,densities)

#the far-most right point means that 100% of the values in samples are less than the x value at that point
abline(h=0.5,v=50)
```
...
# this point corresponds well with my answer earlier showing that 51% of the points in samples fall to the left of the mean (including the mean).. the cumulative density of samples at 50 is slightly higher than the point (50,0.5)
