...
#hist(dnorm(variates, mean = 50, sd = 5))
...
```{r q3}
x_2 <- rnorm(100, mean = 50, sd = 10)
y_2 <- dnorm(x_2, mean = 50, sd = 10)
hist(x_2, freq = FALSE) 

x_3 <- rnorm(100, mean = 50, sd = 100)
y_3 <- dnorm(x_3, mean = 50, sd = 100)
hist(x_3, freq = FALSE) 

#When the standard deviation is adjusted to 10, the distribution becomes slighty left-skewed (negative skewness), whereas when the standard debiation is adjusted to 100, the distribution becomes slightly right-skewed (positive skewness).The distribution is not as normal or even about the mean of 50. 
```
...
q4 = dnorm(x, mean = mean(x), sd = sd(x))
q4
...
plot(x,(pnorm(x, mean = mean(x), sd = sd(x))))


#The far-most right point is the maximal value of the set. 
```
...
#The line here goes straight through the mean. It is the center most point of the graph, thus allowing us to tell more accurately the distribution of the values above or below the 50th percentile (is 50% of the density to the left of the right of the mean). 
