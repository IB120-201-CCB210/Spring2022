...
hist(x, freq = FALSE)
#points(x, y)
#with standard deviation of 100
x <- rnorm(100, mean = 50, sd = 100)
y <- dnorm(x, mean = 50, sd = 100)
hist(x, freq = FALSE)
#points(x, y)
#Increasing the standard deviation from 10 and then 100 seems to increase the spread of values in the distribution.
```
...
```{r q6}
#cumulative densities of same 100 samples
z=pnorm(x,mean=50,sd=5)
plot(x,z)
#The far-most right point is the maximum value found in the distribution of 100 samples. 
```
...
plot
abline(h=0.5, v=50)
#The point in the graph outlined by the gridlines corresponds to the mean of the distribution (in this case 50) which would be the central peak of the PDF and the point of inflection for the CDF. PDF (probability density function) creates the stereotypic bell-curve distribution of probability densities for each bucket of values in the distribution, whereas the CDF(cumulative density function)adds up the densities in a rolling fashion as the values increase in the distribution, creating an 'S' curve that continuously increases from left to right.
```
