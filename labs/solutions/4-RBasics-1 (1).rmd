...
#as the standard deviation increases (histogram of x compared to the histogram of a), the spread of the data increases along with it which also means that the density of individual values along the x axis of the histogram decreases 
```
...
plot(x,pnorm(x,50,5))
abline(h=0.5,v=50)
#the far-right point indicates the cumulative density of all of our samples/the point at which all of the values along our x axis should be equal to or fall below that value, which is why the curve approaches 1 at that point (it is certain that our selected values will be located somewhere at or below the end of this graph's x axis which is our largest value)
```
...
```{r q7}
#the point at the intersection of the gridlines indicates the cumulative probability that a given point would fall at or below 50 (ie the mean of our normal curve's distribution)  -- this corresponds to our calculation in question 2 because here the probability is fifty percent which is the case because we have a perfectly normal distribution centered at 50.
```
