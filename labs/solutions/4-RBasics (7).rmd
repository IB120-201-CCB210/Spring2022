...
hist(x)
#The data becomes more spread out as standard deviation increases, as the domain increases.
```
...
plot(x, y)
abline(h=0.5, v=50)
#The far-most right point represents the sample with highest standard deviation from the mean, and the point which cumulative density reaches 1.
```
...
```{r q7}
# This point matches that of the value found in part 2, with values to the right of the mean corresponding directly to a CDF of 0.5.
```
