...
myName
is.character(myName) #character like string?
```
```{r vectors}
myList <- c(1,2,3) 
myList
...
# matrix index [a,b] two arguments a for row b for column different with list 
```

### Dataframes

```{r dataframes, paged.print=FALSE}
# heterogeneous 和matrix不一样
df <- data.frame(geneIndex = c(1, 2, 3), geneID = c(92231, 2313, 5641), geneName = c("FOS1", "JUN", "BERP"))
df
```

You can extract values from a dataframe similarly to how you would from a matrix. searched by index
...
```{r mtcars}
dimnames(mtcars)[2] # field names 返回field names
...
```{r q3}
sd10 <- rnorm(100, mean = 50, sd = 10)
sd100 <- rnorm(100, mean = 50, sd = 100)
hist(sd10)
hist(sd100)
# The range of date increases with enlargement of standard deviation
```
...
```{r q4}
sd5 <- rnorm(100, mean = 50, sd = 5)
dsd5 <- dnorm(sd5, mean = 50, sd = 5)
hist(sd5)
dsd5 # densities of these random samples
```
...
```{r q6}
cds5 <- pnorm(sd5, mean = 50, sd = 5)
cds5
plot(sd5, cds5)
# the far-most point means almost no sample is bigger than the far-most point and all cumulative possiblities equal to 1
```
...
plot(sd5, cds5)
abline(h=0.5, v=50)
# the midpoint of this figure shows 50% of the density is to the left of the mean which matches the calculating results in part 2.
#relationship between CDF and PDF: the area under the curve of a pdf between negative infinity and x is equal to the value of x on the cdf
```
