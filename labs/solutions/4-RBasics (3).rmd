...
sd_100 <- rnorm(100, 50, 100)
sd_10
sd_100
hist(sd_10)
hist(sd_100)
#as the standard deviation increases, the spread of the data increases (there is a wider range)
```
...
z <- pnorm(x, 50, 5)
plot(x,z)
#the far most right point means when that for that value from our 100 samples, 100% of the rest of the samples lie to the left on the distribution. In other words, this is our maximum value.
```
...
plot(x,z)
abline(h=0.5, v=50)
#They both show the CDF at that point, as they both show that for a value of 50 from our random sample, 50% of the points are equal to or less than that value, while a PDF would tell us the probability that a randomly selected value from our sample will have a specific value.
```
