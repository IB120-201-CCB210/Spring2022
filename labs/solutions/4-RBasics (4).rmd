...
hist(rnorm(100, mean = 50, sd = 100))
#with a larger SD, there is a larger range of data points/spread of the data. 
```
...
```{r q4}
#calling my named samples from above
samples
#calculate probabilities (densities)
dsamples <- dnorm(samples, mean = 50, sd = 5)
```
...
```{r q6}
psamples <- pnorm(samples, mean = 50, sd=5)
plot(samples, psamples)
abline(h=0.5, v=50)
#  A cumulative distribution function (CDF) shows the probability that a random variable will take a value less than or equal to x, which in the graphs below have units of standard deviations. Recall that standard deviations are a measure of the amount of variation in the data.
# This far-most right point of this cumulative density function has the highest probability of being less than or equal to the standard deviation, 5. 
...
```{r q7}
#This point represents where the PDF is at it's peak, the mean of the samples, because that is the highest probability point and half of the density is to the left of that. 
```
