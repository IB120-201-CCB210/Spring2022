...
```{r q3}
x_10 <- rnorm(100, mean = 50, sd = 10) 
y_10 <- dnorm(x_10, mean = 50, sd = 10) 
plot_10 = hist(x_10, freq = FALSE) 
plot_10
# increasing the sd to 10 has increased the spread (or domain), and thus lowered the maximum density values. The plot looks less normally distributed. 
x_100 <- rnorm(100, mean = 50, sd = 100) 
y_100 <- dnorm(x_100, mean = 50, sd = 100) 
plot_100 = hist(x_100, freq = FALSE) 
plot_100
# increasing the sd to 100 has increased the spread (domain) even further, and lowered the maximum density values strongly. The plot looks even less normally distributed.
...
```{r q6}
# cummulative densities pnorm()
x <- rnorm(100, mean = 50, sd = 5)
y <- pnorm(x, mean = 50, sd = 5) 
plot = plot(x,y)
plot
abline(h=0.5, v=50)
```  
...
```{r q7}
# CDF is the cummulative density function. This means that it gives the values less than or equal to a point. (The area) 
# PDF is the probability density function. This means that it gives the value for a specific probability. 
# This graph shows how if we integrate across the entire histogram (cummulative), we will have a density of 1, and if we integrate to the half (50), we should also have about half the cummulative data. 
# To summarize, the point of inflection at x=50 of the CDF should correspond to the mean (peak) of the PDF seen in figure 3.
```
