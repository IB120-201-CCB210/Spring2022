...
```{r q1}
my_samples <- rnorm(100, mean = 50, sd = 5)
hist(my_samples)
```
...
```{r q3}
my_samples_2 <- rnorm(100, mean = 50, sd = 10)
hist(my_samples_2)
my_samples_3 <- rnorm(100, mean = 50, sd = 100)
hist(my_samples_3)
#As the standard deviation gets larger, the data that you can draw gets more variable. They are
#not as centered around the mean value and do not look as "normal" 
#The domain and range of the data becomes more variable with a larger standard deviation 
...
```{r q4}
#Ignorning #4 and #5 per instructions 
```
...
```{r q6}
#calculating based on part 1 
plot(my_samples, pnorm(my_samples, mean = 50, sd = 5))
abline(h=0.5, v=50) 


#the right most point on the graph is the point x where the area
# under the curve up tp that point is the probability that a random variable 
#is equal to or less than that value of x 
#So the farmost right point is the probability that any value in the range will be chosen
#which is the probability of 1 (you will pick a value in the set if you pick a value)
```
...
```{r q7}
#The point outlined by the gridlines is the inflection point in the CDF graph.
#This shows where the 50% probability point is in the values (which is the mean). 
#After the mean value, the probability increases rapidly and then levels out which matches
#the area under the curve of the shape of the normal distribution graph as the values increase
#Corresponsing to the answer in #3, as the standard deviation changes, this point of inflection #(or the halfway point, 50% probability)would not change, but the spread of the CDF would #increase or decrease depending on how spread out the data is due to the variation. 
```
